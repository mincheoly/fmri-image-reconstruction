...
for example = 1 : num_ex
    for pix = 1 : 100
        ...
        predict = max_idx - 1;
        predict_train_stimuli_1x1(pix, example) = predict;
    end
    fprintf('Example %d done for 1x1 \n', example);
end
disp('1x1 done');
...
for example = 1 : num_ex
    for pix = 1 : 90
        ...
        for t = 1 : 5
            predictions(:, t) = predictor_1x2{pix}{t}( decoder_test_data(: , example) );
        end
        predictions = mean(predictions, 2);
        [~, max_idx] = max(predictions);    
        predict = -1;
        ...
        predict_train_stimuli_1x2(pix, example) = predict;
    end
...
for example = 1 : num_ex
    for pix = 1 : 90
        ...
        predictions = zeros(3, 5);
        for t = 1 : 5
            predictions(:, t) = predictor_2x1{pix}{t}( decoder_test_data(: , example) );
        end
        predictions = mean(predictions, 2);
        [~, max_idx] = max(predictions);    
        predict = -1;
        ...
        predict_train_stimuli_2x1(pix, example) = predict;
    end
...
for example = 1 : num_ex
    for pix = 1 : 81
        ...
        for t = 1 : 5
            predictions(:, t) = predictor_2x2{pix}{t}( decoder_test_data(: , example) );
        end
        predictions = mean(predictions, 2);
        [~, max_idx] = max(predictions);    
        predict = -1;
        ...
        predict_train_stimuli_2x2(pix, example) = predict;
    end
